...
import { activeCycle, ownSdp } from '../lib/sdp';
...
checkInsRouter.post('/', asyncRoute(async (req, res) => {
  ...
  const checkInStarts = { Q1: cycle.q1WindowStart, MID_YEAR: cycle.midYearCutoff, Q2: cycle.q2WindowStart, YEAR_END: cycle.yearEndCutoff } as const;
  if (new Date() < checkInStarts[body.period]) throw new ApiError(409, 'CHECKIN_NOT_DUE', 'This check-in is not available yet');
