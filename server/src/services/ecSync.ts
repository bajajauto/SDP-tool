...
import { isBajajAutoEmail } from '../lib/corporateEmail';

export interface EcEmployeeRecord { employeeId: string; fullName: string; email: string; managerEmployeeId?: string; managerEmail?: string; buhrEmployeeId?: string; buhrEmail?: string; bu: string; function: string; department: string; designation: string; jobLevel?: string; positionLevel?: string; company?: string; sector?: string; baseLocation?: string; circle?: string; ro?: string; hub?: string; gender?: string; topPotential?: boolean; buHeadEmployeeId?: string; hireDate: string; isActive: boolean; }
...
export async function runEcSync(provider: EcProvider, actorEmployeeId: string, ipAddress?: string) {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => { for (const record of records) { const corporateEmail = isBajajAutoEmail(record.email); const gaps = [...(!record.managerEmployeeId ? ['MANAGER_NOT_MAPPED'] : []), ...(!record.buhrEmployeeId ? ['BUHR_NOT_MAPPED'] : []), ...(!corporateEmail ? ['NON_CORPORATE_EMAIL'] : [])]; const exists = await tx.employee.findUnique({ where: { employeeId: record.employeeId }, select: { employeeId: true } }); const data = { ...record, isActive: record.isActive && corporateEmail, managerEmployeeId: record.managerEmployeeId ?? null, managerEmail: record.managerEmail ?? null, buhrEmployeeId: record.buhrEmployeeId ?? null, buhrEmail: record.buhrEmail ?? null, buHeadEmployeeId: record.buHeadEmployeeId ?? null, hireDate: new Date(record.hireDate), syncGaps: gaps, lastSyncedAt: new Date() }; await tx.employee.upsert({ where: { employeeId: record.employeeId }, create: data, update: data }); exists ? updatedCount++ : createdCount++; if (gaps.length) flaggedCount++; } });
